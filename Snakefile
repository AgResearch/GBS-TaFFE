...
rule all:
    input:
        ...
        expand('results/04_humann3Uniref50EC/{samples}_kneaddata_pathabundance.tsv', samples = FIDs),
        expand('results/04_centrifugeGTDB/{samples}.GTDB.centrifuge.report', samples = FIDs),
        expand('results/04_kmcpGTDB/{samples}.GTDB.kmcp.report', samples = FIDs),
...
rule kraken2GTDB:
    ...
    log:
        'logs/kraken2/{samples}.kraken2.GTDB.log'
    conda:
        'kraken2'
    threads: 20 
    resources: 
...
rule brackenGTDB:
    ...
    log:
        'logs/bracken/{samples}.bracken.GTDB.log'
    conda:
        'kraken2'
    threads: 2 
    shell:
...
rule humann3Uniref50EC:
    ...
    log:
        'logs/humann3/{samples}.human3.log'
    conda:
        'biobakery'
    threads: 18
    resources:
...
rule kmcpGTDBSearch:
    ...
    log:
        'logs/kmcp/{samples}.kmcp.search.GTDB.log'
    conda:
        'kmcp'
    threads:12
    resources: 
        mem_gb= lambda wildcards, attempt: 104 + ((attempt - 1) * 24),
...
rule kmcpGTDBProfile:
    input:
        ...
        taxid='/bifo/scratch/2022-BJP-GTDB/2022-BJP-GTDB/kmcp/taxid.map',
    output:
        profile='results/04_kmcpGTDB/{samples}.GTDB.kmcp.report',
    log:
        'logs/kmcp/{samples}.kmcp.profile.log'
    conda:
        'kmcp'
    threads: 8 
    resources: 
    ...
    shell:
        ...
        '--mode 1 '
        '--threads {threads} '
        '-X /bifo/scratch/2022-BJP-GTDB/2022-BJP-GTDB/kmcp/gtdb-taxdump/R207 '
        '-T {input.taxid} '
        '-o {output.profile} '
        '--log {log} '
        '{input.kmcpSearch} '
...
rule centrifugeGTDB:
    input:
        KDRs=rules.kneaddata.output.KDRs,
    output:
        report='results/04_centrifugeGTDB/{samples}.GTDB.centrifuge.report',
        out='results/04_centrifugeGTDB/{samples}.GTDB.centrifuge',
    log:
